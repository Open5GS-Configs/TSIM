...
class AnsibleManager(CommandLineManager):
    ...
    def _writeVars(self):
        if(self.runCommand(["git", "ls-remote", self.config["ogs"]["repo"]], noOutput=True) != 0):
    ...
    def _writeInventory(self):
        newInv = INVENTORY.replace("HPLMN_PUBLIC_IP", self.config["hplmn"]["public_ip"])
        newInv = newInv.replace("VPLMN_PUBLIC_IP", self.config["vplmn"]["public_ip"])
