...
class setupTOPSSIM():
    ...
    def setup(self):
        ...
        if not args.provider:
            if config["PROVIDER"].lower() in CLOUD_PROVIDERS:
                ...
                if "VULTR_API_KEY" not in config.keys():
                    self.raiseMissingConfig("VULTR_API_KEY")
                ...
                for p in CLOUD_REQUIRED_PARAMETERS:
                    if p not in config.keys():
                        self.raiseMissingConfig(p)
                
                availPlans = self.getVultrPlans(config['VULTR_API_KEY'])
                idAvailPlans = [plan['id'] for plan in availPlans]
                if config["VULTR_PLAN_ID"] not in idAvailPlans:
                    self.raiseWrongConfig("VULTR_PLAN_ID")

                availRegions = self.getVultrRegions(config['VULTR_API_KEY'])
                idAvailRegions = [region['id'] for region in availRegions]
                for region in [config["H-REGION"], config["V-REGION"]]:
                    if region not in idAvailRegions:
                        self.raiseWrongConfig(region)
        ...
        self.addAnsibleSSHKey(config)

        print(SEPARATOR + f"Calling {strategy.__class__.__name__}" + SEPARATOR)
        strategy.callInfManager(config)


    def addAnsibleSSHKey(self, config):
    ...
    def raiseMissingConfig(self, par):
        errorMsg = f"Required paramater not provided: {par}"
        raise Exception(errorMsg)


    def raiseWrongConfig(self, par):
